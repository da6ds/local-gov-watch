...
export default function BrowseElections() {
  ...
  const {
    ...
    queryFn: async () => {
      let query = supabase.from('election').select('*').gte('date', now.toISOString().split('T')[0]).lte('date', ninetyDaysLater.toISOString().split('T')[0]).order('date', {
        ascending: true
      });
      ...
      if (topicsParam && data) {
        ...
        if (topics.length > 0) {
          const {
            data: topicData
          } = await supabase.from('item_topic').select('item_id').eq('item_type', 'election').in('topic', topics);
          if (topicData) {
  ...
  const getRegistrationStatus = (election: any) => {
    ...
    if (isPast(deadline)) {
      ...
      return <div className="flex items-center gap-2 text-destructive text-sm">
          <AlertCircle className="h-4 w-4" />
  ...
  return <Layout>
      <div className="space-y-6">
        ...
        {isLoading ? <div className="space-y-4">
            {[1, 2].map(i => <Card key={i} className="p-6">
                <Skeleton className="h-6 w-3/4 mb-2" />
                <Skeleton className="h-4 w-full mb-4" />
                <Skeleton className="h-4 w-1/2" />
              </Card>)}
          </div> : elections && elections.length > 0 ? <div className="space-y-4">
            {elections.map(election => <Link key={election.id} to={`/elections/${election.id}`}>
                ...
                <div className="space-y-2">
                  ...
                  {election.info_url && <a href={election.info_url} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:underline inline-block mt-2">
                      More information →
                    </a>}
                </div>
              </Card>
