...
export default function BrowseLegislation() {
  ...
  const {
    ...
    queryFn: async () => {
      let query = supabase.from('legislation').select('*').order('created_at', {
        ascending: false
      }).limit(50);
      ...
      if (topicsParam && data) {
        ...
        if (topics.length > 0) {
          const {
            data: topicData
          } = await supabase.from('item_topic').select('item_id').eq('item_type', 'legislation').in('topic', topics);
          if (topicData) {
  ...
  return <Layout>
      <div className="space-y-6">
        ...
        <Card className="p-4">
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search legislation..." className="pl-10" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            </div>
        ...
        {isLoading ? <div className="space-y-4">
          ...
          </div> : legislation && legislation.length > 0 ? <div className="space-y-4">
            {legislation.map(item => <Link key={item.id} to={`/legislation/${item.id}`}>
                <Card className="p-6 hover:shadow-md transition-shadow cursor-pointer">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold mb-1">{item.title}</h3>
                      {item.summary && <p className="text-muted-foreground text-sm line-clamp-2">
                          {item.summary}
                        </p>}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-muted-foreground mb-3">
                    {item.introduced_at && <div className="flex items-center gap-1">
                        <Calendar className="h-4 w-4" />
                        <span>Introduced {format(new Date(item.introduced_at), 'MMM d, yyyy')}</span>
                      </div>}
                    {item.passed_at && <div className="flex items-center gap-1">
            ...
            {!searchTerm && !topicsParam && <p className="text-sm text-muted-foreground mt-2">
