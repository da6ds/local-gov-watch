...
BEGIN
  IF NOT EXISTS (
    ...
    AND conrelid = 'public.item_topic'::regclass
  ) THEN
    ALTER TABLE public.item_topic 
...
BEGIN
  ...
  CASE window_name
    WHEN '7d' THEN start_time := end_time - INTERVAL '7 days';
    WHEN '30d' THEN start_time := end_time - INTERVAL '30 days';
    WHEN '6m' THEN start_time := end_time - INTERVAL '6 months';
    WHEN '1y' THEN start_time := end_time - INTERVAL '1 year';
    ELSE RAISE EXCEPTION 'Invalid window: %', window_name;
  END CASE;
